...
class Preprocessor:
    def __init__(self):
        self.data = None
        self.mean_norms = {"Gender": 0.5, "Albumin": 4.3, "ALP": 72, "ALT": 29, "AST": 23, "Bilirubin": 0.3,
                           "BUN": 14, "Cholesterol": 190, "Creatinine": 1.0, "FiO2": 0.2, "GCS": 15, "Glucose": 100,
                           "HCO3": 24, "HCT": 45, "HR": 72, "K": 3.9, "Lactate": 0, "Mg": 2, "MAP": 90,
                           "MechVent": 0, "Na": 140, "PaO2": 100, "PaCO2": 40, "pH": 7.4, "Platelets": 300,
                           "RespRate": 15,
    ...
    def pretransform(self, data):
        if data.isna().mean().sum() > 0:
            ...
            for norm in self.mean_norms:
                for col in data.columns:
                    if norm in col:
                        data[col] = data[col].fillna(self.mean_norms[norm])
        return data
